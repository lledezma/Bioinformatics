...
class Test:
	...
	def get_helices(self,line):
		...
		try:
			...
			for PPline in Plines:
				if(PPline[:6] == 'SEQRES' and PPline[10:12].strip() == chain):
					if(lastAA == 0):
						lastAA = int(PPline[12:17].strip())
					else:
						pass
				elif(PPline[:6].strip() == "HELIX" and PPline[19:20].strip() == chain):
					self.secondary.append(PPline)
				elif(PPline[:6].strip() == "SHEET" and PPline[21:22].strip() == chain):
					self.secondary.append(PPline)
		...
		for line in lines:
			if(  line[:4] == 'ATOM'  and line[21:22] == chain):
				if(line[:4] == 'ATOM'  and line[21:22] == chain and line[12:16].strip() == 'C' and int(line[22:26].strip()) == int(aaNumber)-1):
					Cphi.append(float(line[30:38].strip())) #append X
					Cphi.append(float(line[38:46].strip())) #append Y
					Cphi.append(float(line[46:54].strip())) #append Z
				elif(line[:4] == 'ATOM'  and line[21:22] == chain and line[12:16].strip() == 'N' and line[22:26].strip() == aaNumber):
				    N.append(float(line[30:38].strip()))
				    N.append(float(line[38:46].strip()))
				    N.append(float(line[46:54].strip()))
				elif(line[:4] == 'ATOM'  and line[21:22] == chain and line[12:16].strip() == 'CA' and line[22:26].strip() == aaNumber):
				    CA.append(float(line[30:38].strip()))
				    CA.append(float(line[38:46].strip()))
				    CA.append(float(line[46:54].strip()))
				elif(line[:4] == 'ATOM'  and line[21:22] == chain and line[12:16].strip() == 'C' and line[22:26].strip() == aaNumber):
				    C.append(float(line[30:38].strip()))
				    C.append(float(line[38:46].strip()))
				    C.append(float(line[46:54].strip()))
				    aaName = line[17:20].strip()
				elif(line[:4] == 'ATOM'  and line[21:22] == chain and line[12:16].strip() == 'N' and int(line[22:26].strip()) == int(aaNumber)+1):
	...
	def get_torsion(self, Cphi, N, CA, C,Npsi, protein, aaNumber, chain, aaName):
			...
			try:
				torsions['phi'] = get_phi_angle(Cphi, N, CA, C)
			except:
				pass
			try:
				torsions['psi'] = get_psi_angle(N, CA, C, Npsi)
			except:
				pass
			return torsions

	def get_phi_angle(self, Cphi, N, CA, C):
